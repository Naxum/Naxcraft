...
public class NaxcraftWarpgate {
	...
	private void loadFile(){
		...
		file = new File(plugin.filePath + "WarpGates.yml");
		if (!file.exists()){
			file.createNewFile();
			initalizeFile();
		}
		} catch (IOException Ex) {
			System.out.println("Error creating new WarpGates file.");
			return;
		}
	...
	private void initalizeFile(){
		File file = new File(plugin.filePath + "WarpGates.yml");
		try {
			BufferedWriter output = new BufferedWriter(new FileWriter(file));
			output.write("WarpGates: {}");
			output.close();
			
		} catch (Exception Ex) {
			System.out.println("Error writing new WarpGates file.");
		}
	}
	
	public boolean loadWarpGates(){
		List<String>list = config.getKeys("WarpGates");
		...
		for (String id : list){
			...
			boolean on = Boolean.parseBoolean(config.getString(id+".on"));
			...
			List<String> destinations = config.getStringList(id+".destinations", null);
			List<String> whitelist = config.getStringList(id+".whitelist", null);
	...
	public NaxGate getWarpGate(String gateId) {
		for (NaxGate gate : this.gates){
			if(gate.getId().equalsIgnoreCase(gateId))
				return gate;
		}
		return null;
	}
	...
	public boolean runCommand(CommandSender sender, String[] args){
		...
		if(args[0].equalsIgnoreCase("create") || args[0].equalsIgnoreCase("add") || args[0].equalsIgnoreCase("c")){
			...
			for(NaxGate gate : this.gates){
				if(block.getLocation().getBlockX() != gate.getSign().getLocation().getBlockX())
					continue;
				
				if(block.getLocation().getBlockY() != gate.getSign().getLocation().getBlockY())
					continue;
				
				if(block.getLocation().getBlockZ() != gate.getSign().getLocation().getBlockZ())
					continue;
			...
			if(args.length == 1){
				...
				for(NaxGate gate2 : this.gates){
					if(block.getLocation().getBlockX() != gate2.getSign().getLocation().getBlockX())
						continue;
					
					if(block.getLocation().getBlockY() != gate2.getSign().getLocation().getBlockY())
						continue;
					
					if(block.getLocation().getBlockZ() != gate2.getSign().getLocation().getBlockZ())
						continue;
	...
	public boolean handleChange(Block eventBlock){
		for(NaxGate gate : this.gates){
			for(Block block : gate.getPortals()){
				if(eventBlock.getWorld().getName() != block.getLocation().getWorld().getName())
					continue;
				
				if(eventBlock.getLocation().getX() != block.getLocation().getX())
					continue;
				
				if(eventBlock.getLocation().getY() != block.getLocation().getY())
					continue;
				
				if(eventBlock.getLocation().getZ() != block.getLocation().getZ())
					continue;
			...
			if(eventBlock.getWorld().getName() != gate.getSign().getLocation().getWorld().getName())
				continue;
			
			if(eventBlock.getLocation().getX() != gate.getSign().getLocation().getX())
				continue;
			
			if(eventBlock.getLocation().getY() != gate.getSign().getLocation().getY())
				continue;
			
			if(eventBlock.getLocation().getZ() != gate.getSign().getLocation().getZ())
				continue;
	...
	public class NaxGate {
		 ...
		 * @param sign The block of the sign used to create it.
		 * @param frame List of frame blocks
		 * @param portals List of portal blocks
		 */		
		...
		private Location getBottomMostPortalBlock() {
			...
			for(Block portal : this.portalBlocks){
				...
				if(portal.getLocation().getBlockY() < lowestY.getLocation().getBlockY())
					lowestY = portal;
			}
		...
		public void removeDestination(String id){
			if(this.destinations.contains(id))
		...
		public boolean withinDistance(Player player, int distance){
			if(player.getWorld().equals(this.getSign().getWorld())){
				if(Math.sqrt(Math.pow(player.getLocation().getBlockX() - this.signBlock.getLocation().getBlockX(), 2) + Math.pow(player.getLocation().getBlockZ() - this.signBlock.getLocation().getBlockZ(), 2)) <= distance){
					return true;
				}
			}
			return false;
		...
		public void handleTeleport(PlayerMoveEvent event){
			if(withinDistance(event.getPlayer(), 3)){
				handleMsg(event.getPlayer());
				
				if(gateWait.containsKey(event.getPlayer()) || teleportCooldown.containsKey(event.getPlayer())) return;
				
				for(Block portal : this.portalBlocks){
					if(!(event.getPlayer().getLocation().getWorld().getName().equals(portal.getLocation().getWorld().getName())))
						continue;
					
					if(event.getPlayer().getLocation().getBlockX() - portal.getLocation().getBlockX() != 0)
						continue;
					
					if(event.getPlayer().getLocation().getBlockY() - portal.getLocation().getBlockY() != 0)
						continue;
					
					if(event.getPlayer().getLocation().getBlockZ() - portal.getLocation().getBlockZ() != 0)
						continue;
		...
		protected void teleport(PlayerMoveEvent event){
			...
			if(!this.whiteList.isEmpty()){
				if(!this.withinWhiteList(event.getPlayer())){
					if(!plugin.superCommand.isSuper(event.getPlayer().getName().toLowerCase())){
		...
		private boolean withinWhiteList(Player player) {
			for(String name : this.whiteList){
				if(name.startsWith("clan:*")){
					if(plugin.clanCommand.getPlayerClan(player) != null)
						return true;
				}
				
				if(name.startsWith("clan:")){
					String split[] = name.split(":");
					NaxcraftClan clan = plugin.clanCommand.getClan(split[1]);
					
					if(clan == null) continue;
					for(String clanMember : clan.getAllMembers()){
						if(player.getName().equalsIgnoreCase(clanMember)){
							return true;
						}
					}
				}
		...
		public void save(){			
			...
			config.setProperty(id+".name", this.name);
			config.setProperty(id+".signText", this.signText);
			config.setProperty(id+".world", this.signBlock.getWorld().getName());
			config.setProperty(id+".on", this.on);
			
			config.setProperty(id+".sign.x", this.signBlock.getLocation().getBlockX());
			config.setProperty(id+".sign.y", this.signBlock.getLocation().getBlockY());
			config.setProperty(id+".sign.z", this.signBlock.getLocation().getBlockZ());
			
			int i = 0;
			for(Block frame : this.frameBlocks){
				config.setProperty(id+".frame."+i+".x", frame.getLocation().getBlockX());
				config.setProperty(id+".frame."+i+".y", frame.getLocation().getBlockY());
				config.setProperty(id+".frame."+i+".z", frame.getLocation().getBlockZ());
				i++;
			}
			
			i = 0;
			for(Block portal : this.portalBlocks){
				config.setProperty(id+".portals."+i+".x", portal.getLocation().getBlockX());
				config.setProperty(id+".portals."+i+".y", portal.getLocation().getBlockY());
				config.setProperty(id+".portals."+i+".z", portal.getLocation().getBlockZ());
				i++;
			}
			
			config.setProperty(id+".destinations", this.destinations);
			config.setProperty(id+".whitelist", this.whiteList);
			
			i = 0;
			if(this.cost.isEmpty()) {
				List<ItemStack> temp = new ArrayList<ItemStack>();
				temp.add(new ItemStack(Material.AIR, 0));
				config.setProperty(id+".costs.0.material", temp.get(0).getType().name());
				config.setProperty(id+".costs.0.amount", temp.get(0).getAmount());
				
			} else {
				for(ItemStack item : this.cost){
					config.setProperty(id+".costs."+i+".material", item.getType().name());
					config.setProperty(id+".costs."+i+".amount", item.getAmount());
					i++;
				}
			}
